...
def sort_by_entropy(letter_count_df, word_list):
    ...
    for word in word_list:
        ...
        word_values.append(score)
    #sorted_words = [word for _, word in sorted(zip(word_values, word_list))]
    sorted_words = word_list[np.argsort(word_values)]
    #sorted_words.reverse()

    return np.flip(sorted_words)


if __name__ == "__main__":
    w = get_5_letter_words('words.txt')
    dl_w = get_letter_index_frequency(w)

    w = sort_by_entropy(dl_w, w)
